...
const AcceptedOrder=()=>{
    ...
    const UpdateStatus = (id) => {
      ...
      axios.put(   `${config.serverURL}/delivery/updateStatus/${id}/${status}`).then((response) => {
        // setDeliveryBoyList = response.data
        const result=response.data
                        ...
                        <tbody>
                            {orderList.map((order) => {
                               return(<tr>
                                ...
                                <td scope="col"><p>{order.order.address.line1},{order.order.address.line2},
                                {order.order.address.city},{order.order.address.state} {order.order.address.pincode}</p></td>
