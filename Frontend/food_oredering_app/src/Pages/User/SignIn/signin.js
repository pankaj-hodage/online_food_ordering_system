  ...
  let navigate = useNavigate();
    ...
    const signinUser = () => {
        if (email.length === 0) {
            ...
            axios.post(config.serverURL + '/user/signin',{'email':email,'password':password} , {"Content-Type": "application/json"}).then((response) => {
               // alert('i am in axios ')
        ...
        <div style={styles.container}>
          ...
          <div className='mb-3' style={{ marginTop: 30 }}>
            ...
            <div style={{ marginTop: 10 }}>
              Dont have an account yet? <Link  style={{textDecoration:'none'}} to='/signup'>Register here</Link>
            </div>
          </div>
        </div>
      </div>
